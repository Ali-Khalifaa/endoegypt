                    ...
                    <div class="row">
                        ...
                        <div class="col-md-6 mt-4">
                            <div class="custom-toggle-switch d-flex align-items-center mb-4">
                                <input id="toggleswitchPrimary" v-model="data.is_disabled" type="checkbox">
                                <label for="toggleswitchPrimary" class="label-primary"></label><span class="ms-3">{{ $t('global.is_disabled') }}</span>
                            </div>
                            <template v-if="errors['is_disabled']">
                                <error-message v-for="(errorMessage, index) in errors['is_disabled']" :key="index">
                                    {{ errorMessage }}
                                </error-message>
                            </template>
                        </div>
...
export default {
    ...
    setup(props){
       ...
       function defaultData(){
           ...
           submitdata.data.is_disabled = false;
       ...
       function resetModal() {
            ...
            setTimeout(async () => {
                ...
                adminApi.get(`dashboard/categories/${id.value}`)
                    .then((res) => {
                        loading.value = true;
                        let l = res.data.data;
                        ...
                        submitdata.data.is_disabled = l.is_disabled==1;
                    })
                    .catch((err) => {
                        console.log(err);
                    })
    ...
    methods: {
       ...
       this.languages.forEach((el)=>{
        ...
        formData.append('is_disabled', this.data.is_disabled ? 1 : 0);

        if (this.type !== 'edit') {
            if (!this.v$.$error) {
                this.is_disabled = false;
                this.loading = true;
                adminApi.post(`dashboard/categories`, formData)
                    .then((res) => {
                        Swal.fire({
                            icon: 'success',
            ...
            adminApi.post(`dashboard/categories/${this.id}`,formData)
                .then((res) => {
                    Swal.fire({
                        icon: 'success',
