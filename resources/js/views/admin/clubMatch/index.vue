<template>
    <div class="container-fluid">
        ...
        <div class="d-md-flex d-block align-items-center justify-content-between my-4 page-header-breadcrumb">
            <h1 class="page-title fw-semibold fs-18 mb-0">{{ $t('global.clubMatch') }}</h1>
            <div class="ms-md-1 ms-0">
                <nav>
                    <ol class="breadcrumb mb-0">
                        <li class="breadcrumb-item"><router-link :to="{name: 'dashboard'}">{{$t('global.home')}}</router-link></li>
                        <li class="breadcrumb-item active" aria-current="page">{{ $t('global.clubMatch') }}</li>
                    </ol>
                </nav>
            </div>
        </div>
                ...
                <div class="card custom-card">
                    <div class="card-header justify-content-between">
                        ...
                        <div class="prism-toggle">
                            <button v-if="permission.includes('club match create')" @click="showModelCreate" class="btn btn-sm btn-primary-light" data-bs-toggle="modal" data-bs-target="#category-service">
                    ...
                    <div class="card-body">
                        <div class="table-responsive mb-2">
                            <table class="table text-nowrap table-striped">
                                ...
                                <tr>
                                    <th scope="col">#</th>
                                    <th scope="col">{{ $t('global.championship') }}</th>
                                    <th scope="col">{{ $t('global.club1') }}</th>
                                    <th scope="col">{{ $t('global.club2') }}</th>
                                    <th scope="col">{{ $t('global.stadium') }}</th>
                                    <th scope="col">{{ $t('global.match_date') }}</th>
                                    <th scope="col">{{ $t('global.match_link') }}</th>
                                ...
                                <tr v-for="(item,index) in data" :key="item.id">
                                    ...
                                    <td>
                                        <a :href="item.match_link" target="_blank" class="btn btn-icon btn-sm btn-primary-transparent rounded-pill"><i
                                         class="ri-link"></i></a>
                                    </td>
                                    <td>{{item.created_at}}</td>
                                    <td>
                                        <div class="hstack gap-2 fs-15">
                                            <button v-if="permission.includes('club match edit')"
                                            ...
                                            <a href="#" @click.prevent="deleteData(item.id,index)" v-if="permission.includes('club match delete')"
