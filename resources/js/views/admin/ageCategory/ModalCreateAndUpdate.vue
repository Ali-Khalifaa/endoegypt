            ...
            <div class="modal-content">
                <div class="modal-header">
                    <h6 class="modal-title" id="exampleModalLgLabel">
                        {{type == 'create' ? $t('global.addCategory') : $t('global.editCategory')}}
                    </h6>
                ...
                <div class="modal-body">
                    <div class="row">
                        ...
                        <div class="col-md-6">
                            <label for="abbreviation" class="form-label">{{$t('global.abbreviation')}}</label>
                            <input type="text" class="form-control" id="abbreviation" :placeholder="$t('global.abbreviation')"
                                   v-model.trim="v$.abbreviation.$model"
                                   :class="{'is-invalid': v$.abbreviation.$error ||errors[`abbreviation`],
                                'is-valid':!v$.abbreviation.$invalid && !errors[`abbreviation`]}">
                            <div class="invalid-feedback">
                                <span v-if="v$.abbreviation.required.$invalid">{{ $t('validation.fieldRequired') }}<br /> </span>
                            </div>
                            <template v-if="errors['abbreviation']">
                                <error-message v-for="(errorMessage, index) in errors['abbreviation']" :key="index">
                                    {{ errorMessage }}
                                </error-message>
                            </template>
                        </div>
...
export default {
    ...
    setup(props){
       ...
       function defaultData(){
        ...
        let getTypesAndModel = () => {
            ...
            adminApi.get(`dashboard/age-category-enums`)
                .then((res) => {
                    let l = res.data.data;
                    types.value = l.types;
                })
       ...
       function resetModal() {
            ...
            setTimeout(async () => {
                ...
                adminApi.get(`dashboard/age-category/${id.value}`)
                    .then((res) => {
                        loading.value = true;
                        let l = res.data.data;
                        ...
                        submitdata.data.abbreviation = l.abbreviation;
                    })
                    .catch((err) => {
                        console.log(err);
                    })
    ...
    methods: {
       ...
       this.languages.forEach((el)=>{
        ...
        formData.append('abbreviation', this.data.abbreviation);

        if (this.type !== 'edit') {
            if (!this.v$.$error) {
                this.is_disabled = false;
                this.loading = true;
                adminApi.post(`dashboard/age-category`, formData)
                    .then((res) => {
                        Swal.fire({
                            icon: 'success',
            ...
            adminApi.post(`dashboard/age-category/${this.id}`,formData)
                .then((res) => {
                    Swal.fire({
                        icon: 'success',
