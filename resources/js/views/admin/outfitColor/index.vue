<template>
    <div class="container-fluid">
        ...
        <div class="d-md-flex d-block align-items-center justify-content-between my-4 page-header-breadcrumb">
            <h1 class="page-title fw-semibold fs-18 mb-0">{{ $t('global.outfitColor') }}</h1>
            <div class="ms-md-1 ms-0">
                <nav>
                    <ol class="breadcrumb mb-0">
                        <li class="breadcrumb-item"><router-link :to="{name: 'dashboard'}">{{$t('global.home')}}</router-link></li>
                        <li class="breadcrumb-item active" aria-current="page">{{ $t('global.outfitColor') }}</li>
                    </ol>
                </nav>
            </div>
        </div>
                ...
                <div class="card custom-card">
                    <div class="card-header justify-content-between">
                        ...
                        <div class="prism-toggle">
                            <button v-if="permission.includes('outfit color create')" @click="showModelCreate" class="btn btn-sm btn-primary-light" data-bs-toggle="modal" data-bs-target="#category-service">
                        ...
                        <div class="table-responsive mb-2">
                            <table class="table text-nowrap table-striped">
                                ...
                                <tr>
                                    ...
                                    <th scope="col">{{ $t('global.shirt_type') }}</th>
                                    <th scope="col">{{ $t('global.shirt_color') }}</th>
                                    <th scope="col">{{ $t('global.secondary_color') }}</th>
                                    <th scope="col">{{ $t('global.trousers_color') }}</th>
                                ...
                                <tr v-for="(item,index) in data" :key="item.id">
                                    ...
                                    <td>{{item.shirt_type.label}}</td>
                                    <td>
                                        <span class="avatar avatar-md avatar-rounded" :style="'background-color:'+item.shirt_color?.color_code " :title="item.shirt_color?.color_code"></span>
                                    </td>
                                    <td>
                                        <span class="avatar avatar-md avatar-rounded" :style="'background-color:'+item.secondary_color?.color_code " :title="item.secondary_color?.color_code"></span>
                                    </td>
                                    <td>
                                        <span class="avatar avatar-md avatar-rounded" :style="'background-color:'+item.trousers_color?.color_code " :title="item.trousers_color?.color_code"></span>
                                    </td>
                                    <td>
                                    ...
                                    <td>
                                        <div class="hstack gap-2 fs-15">
                                            <button v-if="permission.includes('outfit color edit')"
                                            ...
                                            <a href="#" @click.prevent="deleteData(item.id,index)" v-if="permission.includes('outfit color delete')"
