            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <div class="row">
                        ...
                        <label for="counter" class="form-label">{{$t('global.counter_number')}}</label>
                        <input type="number" class="form-control" id="counter" :placeholder="$t('global.counter_number')"
                            v-model.trim="v$.counter.$model"
                            :class="{'is-invalid': v$.counter.$error ||errors[`counter`],
                            'is-valid':!v$.counter.$invalid && !errors[`counter`]}">
                        <div class="invalid-feedback">
                            <span v-if="v$.counter.required.$invalid">{{ $t('validation.fieldRequired') }}<br /> </span>
                        </div>
                        <template v-if="errors['counter']">
                            <error-message v-for="(errorMessage, index) in errors['counter']" :key="index">
                                {{ errorMessage }}
                            </error-message>
                        </template>
                    </div>
...
export default {
       ...
       function resetModal() {
            ...
            setTimeout(async () => {
                if (props.type != 'edit') {
                    ...
                    adminApi.get(`dashboard/counter/${id.value}`)
                    .then((res) => {
                        loading.value = true;
                        let l = res.data.data;
                        ...
                        submitdata.data.counter = l.counter;
                    })
                    .catch((err) => {
                        console.log(err);
                    })
    ...
    methods: {
       ...
       formData.append('counter', this.data.counter);
        if (this.type !== 'edit') {
            if (!this.v$.$error) {
                this.is_disabled = false;
                this.loading = true;
                adminApi.post(`dashboard/counter`, formData)
                    .then((res) => {
                        Swal.fire({
                            icon: 'success',
            ...
            adminApi.post(`dashboard/counter/${this.id}`,formData)
                .then((res) => {
                    Swal.fire({
                        icon: 'success',
