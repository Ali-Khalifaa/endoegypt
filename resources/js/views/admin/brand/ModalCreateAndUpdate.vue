                    ...
                    <div class="row">
                        ...
                        <div class="col-md-12 mt-3">
                            ...
                            <div class="input-group">
                                <input type="text" class="form-control" :placeholder="$t('global.link')"
                                       v-model.trim="v$.link_url.$model"
                                       :class="{'is-invalid': v$.link_url.$error ||errors[`link_url`],
                                        'is-valid':!v$.link_url.$invalid && !errors[`link_url`]}">
                            </div>
                            <div class="invalid-feedback">

                            </div>
                            <template v-if="errors['link_url']">
                                <error-message v-for="(errorMessage, index) in errors['link_url']" :key="index">
                                    {{ errorMessage }}
                                </error-message>
                            </template>
                        </div>
...
export default {
       ...
       function resetModal() {
            ...
            setTimeout(async () => {
                if (props.type != 'edit') {
                    ...
                    submitdata.data.link_url = props.dataRow.link_url;
    ...
    methods: {
        ...
        formData.append('link_url', this.data.link_url);
        if (this.image) {
        ...
        if (this.type !== 'edit') {
            if (!this.v$.$error && this.numberOfImage) {
                ...
                this.loading = true;
                adminApi.post(`dashboard/brand`, formData)
                    .then((res) => {
                        Swal.fire({
                            icon: 'success',
            ...
            adminApi.post(`dashboard/brand/${this.id}`,formData)
                .then((res) => {
                    Swal.fire({
                        icon: 'success',
