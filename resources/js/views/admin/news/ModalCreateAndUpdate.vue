...
export default {
    ...
    setup(props){
       ...
       function resetModal() {
            ...
            setTimeout(async () => {
                if (props.type != 'edit') {
                    ...
                    adminApi.get(`dashboard/news/${id.value}`)
                    .then((res) => {
                        loading.value = true;
                        let l = res.data.data;
       ...
       function resetModalHidden()
        ...
        return {t,id,
            loading,is_disabled,languages,descRef,handelEditor,
    ...
    methods: {
        ...
        this.languages.forEach((el)=>{
           ...
           formData.append(`details[${el.code}][title]`, 'news details');
        ...
        if (this.type !== 'edit') {
            if (!this.v$.$error && this.numberOfImage) {
                ...
                this.loading = true;
                adminApi.post(`dashboard/news`, formData)
                    .then((res) => {
                        Swal.fire({
                            icon: 'success',
            ...
            adminApi.post(`dashboard/news/${this.id}`,formData)
                .then((res) => {
                    Swal.fire({
                        icon: 'success',
