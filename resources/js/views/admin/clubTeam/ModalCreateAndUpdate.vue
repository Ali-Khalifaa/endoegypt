                ...
                <div class="modal-body">
                    <div class="row">
                        <div class="col-md-6" v-if="data.ar" v-for="lang in languages">
                            <label class="form-label">{{ $t('global.name') }} ({{lang.title}})</label>
                            <input type="text" class="form-control"  v-model="v$[lang.code].title.$model"
                                   :placeholder="$t('global.name')+' '+lang.title"
                        ...
                        <div class="col-md-6 mt-3">
                            <label class="form-label">{{$t('global.abbreviation')}}</label>
                            <div class="input-group">
                                <input type="text" class="form-control" :placeholder="$t('global.abbreviation')"
       ...
       function resetModal() {
            ...
            setTimeout(async () => {
                if (props.type != 'edit') {
                    ...
                    adminApi.get(`dashboard/club-team/${id.value}`)
                    .then((res) => {
                        loading.value = true;
                        let l = res.data.data;
    ...
    methods: {
        ...
        if (this.type !== 'edit') {
            if (!this.v$.$error && this.numberOfImage) {
                ...
                this.loading = true;
                adminApi.post(`dashboard/club-team`, formData)
                    .then((res) => {
                        Swal.fire({
                            icon: 'success',
            ...
            adminApi.post(`dashboard/club-team/${this.id}`,formData)
                .then((res) => {
                    Swal.fire({
                        icon: 'success',
