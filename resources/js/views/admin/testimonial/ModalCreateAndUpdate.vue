                ...
                <div class="modal-body">
                    <div class="row">
                        <div class="col-md-6" v-if="data.ar" v-for="lang in languages">
                            <label class="form-label">{{ $t('global.person_name') }} ({{lang.title}})</label>
                            <input type="text" class="form-control"  v-model="v$[lang.code].title.$model"
                                   :placeholder="$t('global.person_name')+' '+lang.title"
                        ...
                        <div class="col-md-6" v-if="data.ar" v-for="lang in languages">
                            <label class="form-label">{{ $t('global.comment') }} ({{lang.title}})</label>
                            <textarea
                        ...
                        <div class="col-md-6 mt-3">
                            <label class="form-label">{{$t('global.job_title')}}</label>
                            <div class="input-group">
                                <input type="text" class="form-control" :placeholder="$t('global.job_title')"
                        ...
                        <div class="col-md-6 mt-3">
                            <label class="form-label">{{$t('global.rate')}}</label>
                            <div class="input-group">
                                <input type="number" class="form-control" :placeholder="$t('global.rate')"
                                       v-model.trim="v$.rate.$model"
                                       :class="{'is-invalid': v$.rate.$error ||errors[`rate`],
                                        'is-valid':!v$.rate.$invalid && !errors[`rate`]}">
                            </div>
                            <div class="invalid-feedback">
                                <span v-if="v$.rate.required.$invalid">{{ $t('validation.fieldRequired') }}<br /> </span>
                            </div>
                            <template v-if="errors['rate']">
                                <error-message v-for="(errorMessage, index) in errors['rate']" :key="index">
                                    {{ errorMessage }}
                                </error-message>
                            </template>
                        </div>
       ...
       function resetModal() {
            ...
            setTimeout(async () => {
                if (props.type != 'edit') {
                    ...
                    adminApi.get(`dashboard/testimonial/${id.value}`)
                    .then((res) => {
                        loading.value = true;
                        let l = res.data.data;
    ...
    methods: {
        ...
        if (this.type !== 'edit') {
            if (!this.v$.$error && this.numberOfImage) {
                ...
                this.loading = true;
                adminApi.post(`dashboard/testimonial`, formData)
                    .then((res) => {
                        Swal.fire({
                            icon: 'success',
            ...
            adminApi.post(`dashboard/testimonial/${this.id}`,formData)
                .then((res) => {
                    Swal.fire({
                        icon: 'success',
