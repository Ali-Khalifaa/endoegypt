<template>
    <div class="modal fade" id="colors" tabindex="-1"
                ...
                <div class="modal-body">
                    <div class="row">
                        ...
                        <div class="col-md-6 mt-4">
                            <label for="exampleColorInput" class="form-label">{{$t('label.color_code')}}</label>
                            <input type="color" class="form-control form-control-color border-0"
                                   id="exampleColorInput" value="#136ad0" title="Choose your color"
                                   v-model.trim="v$.color_code.$model"
                                   :class="{'is-invalid': v$.color_code.$error ||errors[`color_code`],
                                'is-valid':!v$.color_code.$invalid && !errors[`color_code`]}">

                            <div class="invalid-feedback">
                                <span v-if="v$.color_code.required.$invalid">{{ $t('validation.fieldRequired') }}<br /> </span>
                            </div>
                            <template v-if="errors[`color_code`]">
                                <error-message v-for="(errorMessage, index) in errors[`color_code`]" :key="index">
                                    {{ errorMessage }}
                                </error-message>
                            </template>
                        </div>
...
export default {
    ...
    setup(props){
        setTimeout(async () => {
            let myModalEl = document.getElementById('colors')
       ...
       function resetModal() {
            ...
            setTimeout(async () => {
                ...
                adminApi.get(`dashboard/color/${id.value}`)
                    .then((res) => {
                        loading.value = true;
                        let l = res.data.data;
                        ...
                        submitdata.data.color_code = l.color_code;
                    })
                    .catch((err) => {
                        console.log(err);
                    })
    ...
    methods: {
       ...
       this.languages.forEach((el)=>{
        ...
        formData.append('color_code', this.data.color_code);
        if (this.type !== 'edit') {
            if (!this.v$.$error) {
                this.is_disabled = false;
                this.loading = true;
                adminApi.post(`dashboard/color`, formData)
                    .then((res) => {
                        Swal.fire({
                            icon: 'success',
            ...
            adminApi.post(`dashboard/color/${this.id}`,formData)
                .then((res) => {
                    Swal.fire({
                        icon: 'success',
