                ...
                <div class="modal-body">
                    <div class="row">
                        <div class="col-md-12 mt-3">
                            <label for="video_url" class="form-label">{{$t('global.video_url')}}</label>
                            <input type="text" class="form-control" id="video_url" :placeholder="$t('global.video_url')"
                                v-model.trim="v$.video_url.$model"
                                :class="{'is-invalid': v$.video_url.$error ||errors[`video_url`],
                                'is-valid':!v$.video_url.$invalid && !errors[`video_url`]}">
                            <div class="invalid-feedback">
                                <span v-if="v$.video_url.required.$invalid">{{ $t('validation.fieldRequired') }}<br /> </span>
                            </div>
                            <template v-if="errors['video_url']">
                                <error-message v-for="(errorMessage, index) in errors['video_url']" :key="index">
                                    {{ errorMessage }}
                                </error-message>
                            </template>
                        </div>
       ...
       function resetModal() {
            ...
            setTimeout(async () => {
                if (props.type != 'edit') {
                    ...
                    submitdata.data.video_url = props.dataRow.video_url;
    ...
    methods: {
        ...
        if (this.type !== 'edit') {
            if (!this.v$.$error && this.numberOfImage) {
                ...
                this.loading = true;
                adminApi.post(`dashboard/latest-video`, formData)
                    .then((res) => {
                        Swal.fire({
                            icon: 'success',
            ...
            adminApi.post(`dashboard/latest-video/${this.id}`,formData)
                .then((res) => {
                    Swal.fire({
                        icon: 'success',
