        ...
        <div class="row gy-3">
            ...
            <div class="col-md-6 mb-4">
                ...
                <template v-if="errors['phone']">
                   ...
                   <div class="col-md-6 mb-4">
                <label class="form-label" style="margin-top: 20px;">
                    {{ $t('global.affiliation') }}
                    <span class="text-danger">*</span>
                </label>
                <input type="text" class="form-control form-control-lg" v-model="v$.data.affiliation.$model" :class="{
                    'is-invalid': v$.data.affiliation.$error || errors[`affiliation`],
                    'is-valid': !v$.data.affiliation.$invalid && !errors[`affiliation`]
                }">

                <div class="invalid-feedback"  v-show="v$.data.affiliation.$error || errors[`affiliation`]">
                    <span v-if="v$.data.affiliation.required.$invalid">{{ $t('validation.fieldRequired')
                    }}<br />
                    </span>

                </div>
                <template v-if="errors[`affiliation`]">
                    <error-message v-for="(errorMessage, index) in errors[`affiliation`]" :key="index">
                        {{ errorMessage }}
                    </error-message>
                </template>
            </div>
            ...
            <div class="col-md-6 mb-4">
                ...
                <select class="form-control form-control-lg" v-model="v$.data.role.$model" :class="{
                    'is-invalid': v$.data.role.$error || errors[`role`],
                    'is-valid': !v$.data.role.$invalid && !errors[`role`]
                }">
                    <option value="Speaker">{{ $t('global.Speaker') }}</option>
                    <option value="Attendee">{{ $t('global.Attendee') }}</option>
                    <option value="Student">{{ $t('global.Student') }}</option>
                    <option value="Sponsor">{{ $t('global.Sponsor') }}</option>
                </select>

                <div class="invalid-feedback" v-show="v$.data.role.$error || errors[`role`]">
                    <span v-if="v$.data.role.required.$invalid">{{ $t('validation.fieldRequired') }}</span>
                </div>
                <template v-if="errors[`role`]">
                    <error-message v-for="(errorMessage, index) in errors[`role`]" :key="index">
                        {{ errorMessage }}
                    </error-message>
                </template>
            </div>
            ...
            <div class="col-xl-12 d-grid mt-2 text-center">
                <button type="button" @click.prevent="AddSubmit" class="btn btn-lg btn-primary hvr-float-shadow" v-if="!loading"
                    style="background-color: #fd580b;margin-top: 50px;padding:15px">{{
...
export default {
    ...
    setup(props) {
        ...
        onMounted(()=>{
            submitdata.data.event_id = props.event_id
        })
        setTimeout(() => {
            is_valid.value = true;
        }, 200);
    ...
    methods: {
        AddSubmit() {
            ...
            if (!this.v$.$error && this.is_valid) {
                ...
                formData.append('event_id', this.data.event_id);
                formData.append('affiliation', this.data.affiliation);
                formData.append('role', this.data.role);
                formData.append('email', this.data.email);
                formData.append('phone', `+${this.data.code_country}` + this.data.phone);

                this.loading = true;
                webApi.post(`/web/event-register`, formData)
                    .then((res) => {
                        Swal.fire({
                            icon: 'success',
